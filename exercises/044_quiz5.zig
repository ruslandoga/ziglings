...
fn recursiveVisitElephants(e: *Elephant) void {
    if (e.visited) return;
    e.visited = true;

    std.debug.print("Elephant {u}. ", .{e.letter});
    recursiveVisitElephants(e.tail);
}

fn callVisitElephants(e: *Elephant) void {
    if (e.visited) return;
    e.visited = true;

    std.debug.print("Elephant {u}. ", .{e.letter});
    @call(.{}, callVisitElephants, .{e.tail});
}
